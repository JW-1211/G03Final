...
with tab1:
    st.header("📖 Story")
    st.markdown(f"<div style='font-size: 20px; line-height: 1.8'>{highlight_keywords(text, highlight_words)}</div>", unsafe_allow_html=True)

with tab2:
    st.header("🔤 Translation")
    st.markdown(f"<div style='font-size: 19px; line-height: 1.8'>{translation}</div>", unsafe_allow_html=True)
...
with tab3:
    ...
    if st.button("▶️ Play Audio"):
        ...
        st.markdown(f"**Selected sentence:** {sentence_text}**")
